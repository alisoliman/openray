...
Optionally set the environment variable `OPENRAY_SIGNING_IDENTITY` to the exact full Developer ID Application name to select. Otherwise, the signing helper requires exactly one valid imported Developer ID Application identity. Secrets are imported into a temporary runner Keychain, used for signing and notarization, and removed during cleanup. The helper adds that Keychain to the runner's search list while preserving existing entries and the default Keychain. It signs and verifies a disposable executable before building the app; listing a certificate alone does not prove that `codesign` can use it. Cleanup deletes the temporary Keychain and removes its search-list entry. Do not put certificates, private keys, passwords, or generated Keychains in Git, workflow artifacts, or release assets.
